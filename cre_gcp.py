...
#GOOGLE_APPLICATION_CREDENTIALS = config_data["gcp"]["credentials"]
GCP_PROJECT_ID = "YOUR_GCP_PROJECT_ID"
GCP_DATASET_ID = "gst_data"
...
GCS_CREDENTIALS = service_account.Credentials.from_service_account_info(config_data["gcp"]["credentials"])

# Initialize GCP clients with loaded credentials
storage_client = storage.Client(project=PROJECT_ID, credentials=GCS_CREDENTIALS)
client = bigquery.Client(project=PROJECT_ID, credentials=GCS_CREDENTIALS)

def upload_file_to_gcs(file_name, bucket_name, gcs_file_path):
...
# def run_bigquery_query(project_id, dataset_id, query):
#    """
...
#    Returns:
#        pandas.DataFrame: Query results as DataFrame, or None if query fails
#    """
#    try:
...
#            retry=retry.Retry(deadline=300)  # 5 min timeout
#        )
...
#            print(f"Query billed {query_job.total_bytes_billed/(1024*1024*1024):.2f} GB")
...
#    except Exception as e:
#        print(f"Failed to initialize query: {str(e)}")
#        return None
...
def run_bigquery_query(project_id, dataset_id, query):
    ...
    try:
        ...
        job_config = bigquery.QueryJobConfig(
            use_query_cache=True,
            labels={'source': 'python_client'}
        )

        # Start query job
        query_job = client.query(query, job_config=job_config, retry=retry.Retry(deadline=300))
        ...
        if 'query_job' in locals() and query_job.errors:
            for error in query_job.errors:
                print(f"BigQuery error: {error['message']}")
        return None

def calculate_days_delayed(dof, return_period, frequency):
    # Convert return_period to the correct format if necessary
    if len(return_period) == 6:  # Assuming format is 'MMYYYY'
        return_period = f"{return_period[2:]}-{return_period[:2]}-01"  # Convert to 'YYYY-MM-DD'

    return_period_date = datetime.strptime(return_period, '%Y-%m-%d')
...
def get_data_from_bigquery(gstin):
   """
   Get GST returns data from BigQuery for given GSTIN
   
   Args:
       gstin (str): GSTIN number to query
       project_id (str): GCP project ID
   
   Returns:
       pandas.DataFrame: Processed data frame with GST returns
   """
   try:
       # Initialize BigQuery client
       ...
       query = """
       SELECT * 
       FROM `YOUR_GCP_PROJECT_ID.gst_data.v_gst_returns`
       WHERE ptgstin = @gstin
       """

       job_config = bigquery.QueryJobConfig(
           query_parameters=[
               bigquery.ScalarQueryParameter("gstin", "STRING", gstin)
           ]
       )

       # Execute query
       query_job = client.query(query, job_config=job_config)
       print(query_job, "this is the error")
       # Get results as DataFrame
       df = query_job.to_dataframe()

       if not df.empty:
           # Process data - sort and deduplicate
           df = df.sort_values(
               by=['rtntype', 'ret_prd', 'dof'], 
               ascending=[False, False, False]
           )
           df = df.drop_duplicates(
               subset=['rtntype', 'ret_prd'], 
               keep='first'
           )
           
           print(f"Retrieved and processed {len(df)} records for GSTIN: {gstin}")
           return df
       else:
           print(f"No data found for GSTIN: {gstin}")
           return pd.DataFrame()

   except Exception as e:
       print(f"Error retrieving data for GSTIN {gstin}: {str(e)}")
       return pd.DataFrame()
...
def process_gstin(gstin: str) -> dict:
    """Process data for a single GSTIN."""

    try:
        # Fetch data from Bigquery
        results = get_data_from_bigquery(gstin)
        if results.empty:
            print(f"No data found for GSTIN: {gstin}")
            return None
        ...
        if analysis is None:
            print(f"Could not analyze frequency for GSTIN: {gstin}")
            return None
        ...
        print(f"Error processing GSTIN {gstin}: {str(e)}")
        return None
...
def get_final_compliance_scores():
    ...
    if not borrower_gst or not trader_gst:
        return jsonify({'error': 'Both borrower_gst and trader_gst are required.'}), 400

    # Process the GSTINs and store the results
    borrower_results = process_gstin(borrower_gst)
    trader_results = process_gstin(trader_gst)

    # Return the results as a JSON response
    return jsonify({
        'borrower_gst': borrower_results,
...
def calculate_recency_invoice_tally(borrower_gst, trader_gst):
   """
   Calculate invoice recency tally using BigQuery
   
   Args:
       borrower_gst (str): Borrower's GST number
       trader_gst (str): Trader's GST number
       project_id (str): GCP project ID
   
   Returns:
       dict: Dictionary with invoice counts for different time periods
   """
   ...
   WITH MonthlyData AS (
       ...
       FROM  `YOUR_GCP_PROJECT_ID.pft_prod_erp.v_rpt_tally`
   ...
   SELECT
       MD.ptgstin,
       MD.ctin,
       SUM(CASE WHEN MM.max_month - (MD.year * 12 + MD.month) BETWEEN 0 AND 2 
           THEN MD.num_invoices ELSE 0 END) AS `1_3_months`,
       SUM(CASE WHEN MM.max_month - (MD.year * 12 + MD.month) BETWEEN 3 AND 5 
           THEN MD.num_invoices ELSE 0 END) AS `2_3_months`,
       SUM(CASE WHEN MM.max_month - (MD.year * 12 + MD.month) BETWEEN 6 AND 8 
           THEN MD.num_invoices ELSE 0 END) AS `3_3_months`,
       SUM(CASE WHEN MM.max_month - (MD.year * 12 + MD.month) BETWEEN 9 AND 11 
           THEN MD.num_invoices ELSE 0 END) AS `4_3_months`
   FROM MonthlyData MD
   ...
   job_config = bigquery.QueryJobConfig(
       query_parameters=[
           bigquery.ScalarQueryParameter("borrower_gst", "STRING", borrower_gst),
           bigquery.ScalarQueryParameter("trader_gst", "STRING", trader_gst)
       ]
   )
   ...
   try:
       ...
       # Extract values if results exist
           ...
           for column in recency_invoice_dict.keys():
               if column in result_df.columns:
                   try:
                       ...
                       print(f"Warning: Cannot convert value in '{column}' to integer. Using default 0. Error: {e}")
...
def calculate_average_invoice_tally(borrower_gst, trader_gst):
    ...
    SELECT ptgstin, ctin, COUNT(id) AS NumberOfInvoices,
       SUM(CAST(val AS FLOAT64)) AS TotalInvoiceAmount,
       AVG(CAST(val AS FLOAT64)) AS AvgInvoiceAmount
    FROM `YOUR_GCP_PROJECT_ID.pft_prod_erp.v_rpt_tally`
    WHERE ptgstin = '{borrower_gst}' AND ctin = '{trader_gst}' AND inv_typ ='Sales'
    AND (
        (EXTRACT(YEAR FROM date) * 12 + EXTRACT(MONTH FROM date)) >= ((EXTRACT(YEAR FROM CURRENT_DATE) - 1) * 12 + EXTRACT(MONTH FROM CURRENT_DATE))
    )
    ...
    if 'AvgInvoiceAmount' in result.columns:
...
FROM
    `YOUR_GCP_PROJECT_ID.pft_prod_erp.v_rpt_tally`
CROSS JOIN
    MaxMonth
WHERE
    ptgstin = '{borrower_gst}' -- Replace with your specific pgstin
...
GROUP BY
    ...
    try:
        # Extract the first value from the 'Number of Invoices' column.
        ...
        # Handle the case where the 'Number of Invoices' column is empty.
        ...
        # Handle the case where the 'Number of Invoices' column does not exist.
...
def calculate_recency_invoice(borrower_gst: str, trader_gst: str) -> dict:
    # SQL query for recency of invoices with proper BigQuery syntax
    query = f"""
    WITH MonthlyData AS (
        SELECT
            ptgstin,
        ...
        FROM `YOUR_GCP_PROJECT_ID.gst_data.v_gstr1b2b`
        WHERE ptgstin = @borrower_gst 
    ...
    SELECT
        ...
        MD.ctin,
        SUM(CASE 
            WHEN max_month - (MD.year * 12 + MD.month) BETWEEN 0 AND 2 
            THEN MD.num_invoices ELSE 0 
        END) AS first_quarter,
        SUM(CASE 
            WHEN max_month - (MD.year * 12 + MD.month) BETWEEN 3 AND 5 
            THEN MD.num_invoices ELSE 0 
        END) AS second_quarter,
        SUM(CASE 
            WHEN max_month - (MD.year * 12 + MD.month) BETWEEN 6 AND 8 
            THEN MD.num_invoices ELSE 0 
        END) AS third_quarter,
        SUM(CASE 
            WHEN max_month - (MD.year * 12 + MD.month) BETWEEN 9 AND 11 
            THEN MD.num_invoices ELSE 0 
        END) AS fourth_quarter
    FROM MonthlyData MD
    JOIN MaxMonth MM ON MD.ptgstin = MM.ptgstin 
    ...
    try:
        ...
        if not df.empty:
            # Map BigQuery results to output dictionary
            mapping = {
                "first_quarter": "1_3_months",
                "second_quarter": "2_3_months",
                "third_quarter": "3_3_months",
                "fourth_quarter": "4_3_months"
            }
            
            for bq_col, dict_key in mapping.items():
                try:
                    value = int(df[bq_col].iloc[0])
                    recency_invoice_dict[dict_key] = value
                except (ValueError, TypeError, IndexError) as e:
                    print(f"Warning: Error processing column {bq_col}: {str(e)}")

        return recency_invoice_dict

    except Exception as e:
        print(f"Error executing BigQuery: {str(e)}")
        return None
...
def calculate_average_invoice_amount(borrower_gst, trader_gst):
    ...
    FROM
        `YOUR_GCP_PROJECT_ID.gst_data.v_gstr1b2b`
    WHERE
        (EXTRACT(YEAR FROM `DATE`) * 12 + EXTRACT(MONTH FROM `DATE`)) >= ((EXTRACT(YEAR FROM CURRENT_DATE) - 1) * 12 + EXTRACT(MONTH FROM CURRENT_DATE))
...
def calculate_invoice_to_cash_flow(borrower_gst):
    ...
    FROM `YOUR_GCP_PROJECT_ID.gst_data.v_gst_cashflow` 
    WHERE ptgstin ='{borrower_gst}'
    )
    SELECT yt.ptgstin, yt.year_month, yt.invoice_to_cashflow 
    FROM YOUR_GCP_PROJECT_ID.gst_data.v_gst_cashflow` yt
    JOIN DateRange dr
...
FROM (
    SELECT ptgstin, ctin, inum AS inv_no,PARSE_DATE('%d-%m-%Y', idt) as inv_date,val,txval
    FROM `YOUR_GCP_PROJECT_ID.gst_data.v_gstr1b2b`
) as subquery
...
def fetch_tally_data(borrower_gst, trader_gst):
    ...
    ctin, 
    inv_no, 
    ABS(CAST(val AS FLOAT64)) as val, 
    ABS(CAST(taxval AS FLOAT64)) as taxval, 
    "DATE" as inv_date
FROM `YOUR_GCP_PROJECT_ID.pft_prod_erp.v_rpt_tally`
WHERE 
...
def get_most_recent_gst_date(borrower_gst):
    query = f"""
    SELECT MAX(PARSE_DATE('%d-%m-%Y', idt)) AS recent_date
    FROM `YOUR_GCP_PROJECT_ID.gst_data.v_gstr1b2b`
    WHERE ptgstin ='{borrower_gst}'
    """
    from google.cloud import bigquery
...
def compute_credit_score_api():
    ...
    # Extract the score value from the dictionary
    # Ensure borrower_compliance_score is not None
    if borrower_compliance_score is not None:
        borrower_compliance_score_value = borrower_compliance_score.get('score', 0)
    else:
        borrower_compliance_score_value = 0

    if trader_compliance_score is not None:
        trader_compliance_score_value = trader_compliance_score.get('recommendations', 0)
    else:
        trader_compliance_score_value = 0
        
    if borrower_compliance_score_value < 7 or trader_compliance_score_value < 7:
        ...
        low_scores = []
        if borrower_compliance_score_value < 7:
            low_scores.append(f"Borrower : {borrower_compliance_score_value}")
        if trader_compliance_score_value < 7:
            low_scores.append(f"Trader : {trader_compliance_score_value}")
        
        message = "Cannot calculate credit score due to low compliance score of " + ", ".join(low_scores)
        
        print(borrower_compliance_score,trader_compliance_score,"Error here")
...
# Before setting up transaction_log_entry, check if tally_recent_date is None
        ...
        gcs_file_path = f"credit_recommendation_results/year={year}/month={month}/day={day}/{file_name}"
